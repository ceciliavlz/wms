...
public class OrdenIngreso extends javax.swing.JPanel {
     ...
     */
    public OrdenIngreso(PrincipalJFrame frame) {
        this.frame = frame;
        iniciar();
    }
    ...
    // Rellena el select de racks
    private void cargarRacks(){
        racksId = NaveController.getInstance().listarTodosLosRacks();
        ...
        for (int idrack : racksId){
            int idNaveAct = NaveController.getInstance().naveDeRack(idrack);
            racksComboBox.addItem("NAVE " + idNaveAct + ", ID " + idrack);
        }
    }
    
    // Rellena el select de productos
    private void cargarProductos() {
        List<String> prods = ProductoController.getInstance().listarProductos();
    ...
    private List<String> buscarUbicacionesLlenas(){
        List<String> ubicacionesLista = NaveController.getInstance().calcularPesosRack(idRack);
        List<String> ubicacionesLlenas = new ArrayList();

        Collections.sort(ubicacionesLista);

        String[] ubicaciones = ubicacionesLista.toArray(String[]::new);
        
        int index = 0;
        
        for (int i = 1; i <= 3; i++){
            for (int j = 1; j <= 3; j++){
                String cod = ubicaciones[index].split(" ")[0].split(":")[0];
                
                if (NaveController.getInstance().ubicacionLlena(cod)) {
                    ubicacionesLlenas.add("f" + i + "c" + j);
                }
        ...
        return ubicacionesLlenas;
    }
    
    private void pintarUbicacionesLlenas(){
        for (String u : buscarUbicacionesLlenas()){
            JPanel panel = buscarPanelPorNombreEnPanel(panelUbicaciones, u);
    ...
    public JPanel buscarPanelPorNombreEnPanel(Container container, String nombreBuscado) {
        for (Component comp : container.getComponents()) {
            ...
            if (comp instanceof Container container1) {
                JPanel resultado = buscarPanelPorNombreEnPanel(container1, nombreBuscado);
    ...
    // Cambia color de ubicación elegida
    private void seleccionBoton(JPanel nuevaSeleccion){
        if (ubicacionElegida == null) return;
        
        boolean llena = NaveController.getInstance().ubicacionLlena(ubicacionElegida);
            
        if (llena){
            botonUbicacionElegido = null;
            return;
        }

        if (botonUbicacionElegido != null){
    ...
    private String crearOrdenIngreso(int cantidadCargada){
        
        String respuesta = MovimientoController
                .getInstance()
                .generarOrdenIngresoFechaIngresada(
                        idProducto, 
                        ubicacionElegida, 
                        cantidadCargada, 
                        fecha
    ...
    private void botonCrearOrdenActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botonCrearOrdenActionPerformed
        String cantText = cantidad.getText().replaceAll(",", "");
        ...
        int cantidadCargada = Integer.parseInt(cantidad.getText());
        
        System.out.print(cantidadCargada);

        String res = crearOrdenIngreso(cantidadCargada);
